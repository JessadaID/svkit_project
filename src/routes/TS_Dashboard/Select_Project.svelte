<script>
  ...
  let saving = false;
  // let saveMessage = ""; // Remove saveMessage, as we'll use toast instead
  ...
  onMount(async () => {
    try {
      ...
      if (currentUser) {
        ...
        // Try to get user's display name or fetch from user collection if available
        userName = currentUser.displayName || "";

        // If display name is not available, try to fetch from users collection
        if (!userName) {
          try {
            const userDoc = await getDoc(doc(db, "users", userEmail));
            if (userDoc.exists()) {
              userName = userDoc.data().name || userEmail;
            } else {
              userName = userEmail; // Fallback to email if no name found
            }
          } catch (err) {
      ...
      if (res.ok) {
        ...
        projects = projects.map((project) => {
          // Check if current user is already a director
          const directors = project.directors || {};
          const isDirector = directors[userName] !== undefined;
          return {
            ...project,
      ...
      dangerToast("เกิดข้อผิดพลาดในการโหลดข้อมูลโครงงาน กรุณาลองใหม่อีกครั้ง");
    }finally{
      loading = false;
    }
  });
  ...
  function applyFilters() {
    filteredProjects = projects.filter((project) => {
      ...
      if (searchQuery.trim() !== "") {
        ...
        const adviserMatch =
          project.adviser &&
          project.adviser.some((member) => member.toLowerCase().includes(query));
  ...
  async function saveSelection() {
    ...
    try {
      ...
      const updatePromises = updatedProjects.map(async (project) => {
        ...
        if (project.selected) {
          // Add teacher to directors with timestamp if not already there
          if (!currentDirectors[userName]) {
            const updatedDirectors = {
              ...currentDirectors,
              [userName]: {
                // สามารถเพิ่มข้อมูลอื่นๆ ได้ในอนาคต เช่น
                // role: "committee",
                // notes: ""
              }
            };
          ...
          // Remove teacher from directors if they were previously selected
          if (currentDirectors[userName]) {
            const { [userName]: removed, ...remainingDirectors } =
              currentDirectors;

            await updateDoc(projectRef, {
              directors: remainingDirectors,
            });
          }
        }
      });
        ...
        <div class="flex flex-col md:flex-row gap-4">
          ...
          <div class="w-full md:w-1/3">
            ...
            <select
              ...
              class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm py-2 px-3 border"
            >
              {#each availableTerms as term}
                <option value={term}
                  >{term === "all" ? "ทุกภาคการศึกษา" : term}</option
                >
              {/each}
            </select>
          </div>
            ...
            >
              <div class="flex justify-between items-start">
                <div>
                  ...
                  <p class="mt-2">
                    อาจารย์ที่ปรึกษา: {project.adviser?.join(', ') || "ไม่ระบุ"}
                  </p>

                  {#if project.directors && Object.keys(project.directors).length > 0}
                    <div class="mt-2">
                      <p class="font-medium">กรรมการ:</p>
                      <ul class="list-disc list-inside ml-4">
                        {#each Object.keys(project.directors) as director}
                          <li>{director}</li>
                        {/each}
                      </ul>
                    </div>
