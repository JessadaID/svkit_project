<script>
    ...
    onMount(async () => {
      userName = getCookie("name") || null; // Get teacher's name from cookies
      try {
        loading = true;
        ...
        if (res.ok) {
          ...
          projects = projects.map((project) => {
            ...
            const isDirector = directors[userEmail] !== undefined;
            return {
              ...project,
      ...
      try {
        ...
        const updatePromises = updatedProjects.map(async (project) => {
          ...
          if (project.selected) {
            ...
            if (currentDirectors[userEmail]) {
              const { [userEmail]: removed, ...remainingDirectors } =
                currentDirectors;
