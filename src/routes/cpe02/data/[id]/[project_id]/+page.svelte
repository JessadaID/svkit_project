<script>
  ...
  let can_edit_task = false; // This should likely be determined based on role/email
  ...
  function goToEditPage() {
    goto(`/cpe02/edit/${projectId}`); // Adjust path as needed
  }
  ...
  onMount(async () => {
    ...
    try {
      ...
      if (projectSnap.exists()) {
        ...
        can_edit_task = role === 'admin' || isAdvisor;

        status = [];
        comment = [];
        if (project.Tasks && typeof project.Tasks === 'object') {
             Object.entries(project.Tasks).forEach(([key, task]) => {
                const index = parseInt(key, 10); // Assuming keys are numeric indices '0', '1', ...
                if (!isNaN(index)) {
                    comment[index] = task.comment || ""; // Ensure default value
                    status[index] = task.status || "";   // Ensure default value
                }
            });
        }
        ...
        visibleStates = Task.map(() => false);
         ...
         // Ensure status and comment arrays have enough length, padding with defaults if needed
         const requiredLength = Task.length;
         while (status.length < requiredLength) status.push("");
         while (comment.length < requiredLength) comment.push("");
